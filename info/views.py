...
def marks_list(request, stud_id):
    ...
    for ass in ass_list:
        try:
            ...
            sc.marks_set.create(type='A', name='Assignment')
            sc.marks_set.create(type='T', name='Tutorial')
            #sc.marks_set.create(type='C', name='Total Internal Marks')
...
@login_required()
def generate_report(request, assign_id):
    ass = get_object_or_404(Assign, id=assign_id)
    response = HttpResponse(content_type='text/csv')  
    response['Content-Disposition'] = 'attachment; filename='+ str(ass.course) + ".csv"
    writer = csv.writer(response) 
    writer.writerow([ass.course])
    for stud in ass.class_id.student_set.all():
        a = StudentCourse.objects.get(student=stud, course=ass.course)

        marks_list = a.marks_set.all()
        m = []
        for mk in marks_list:
            m.append(mk.marks1)
        cie = math.ceil((m[0] / 2) + (m[1]/ 2)) + sum(m[2:4])

        att = AttendanceTotal.objects.get(student=a.student, course=a.course).attendance

        writer.writerow([a.student_id,a.student.name,cie,att])
    return response
